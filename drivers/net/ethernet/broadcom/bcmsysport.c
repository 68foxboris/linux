				   ...
				   netdev_features_t wanted)
{
	 ...
	 * tag after the Ethernet MAC Source Addresss.
	 */
					     ...
					     struct bcm_sysport_tx_ring *ring)
{
	...
	netif_dbg(priv, tx_done, ndev,
		"ring=%d p_index=%d c_index=%d last_tx_cn=%d last_c_index=%d\n",
		ring->index, ring->p_index, c_index, last_tx_cn, last_c_index);
...
static irqreturn_t bcm_sysport_tx_isr(int irq, void *dev_id)
{
	...
	for (ring = 0; ring < dev->num_tx_queues; ring++) {
		...
		bcm_sysport_tx_reclaim(priv, &priv->tx_rings[ring]);
	}
				    ...
				    struct net_device *dev)
{
	...
	/* lock against tx reclaim in interrupt context */
	spin_lock_irqsave(&ring->lock, flags);
	 ...
	 * the switch port logic. When Broadcom tags are enabled, we need to
	 * make sure that packets are at least 68 bytes because the length
	 * verification is used after the Broadcom tag is removed.
	 */
					...
					unsigned int index)
{
	...
	/* ring->cbs is the last part of bcm_sysport_tx_init() which could fail
	 * so by checking this pointer we know whether the TX ring was fully
	 * initialized or not
	 */
	if (!ring->cbs)
		return;
