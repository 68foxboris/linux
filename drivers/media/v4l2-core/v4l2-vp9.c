       ...
       for (i = 0; i < ARRAY_SIZE(probs->coef); i++) {
               for (j = 0; j < ARRAY_SIZE(probs->coef[0]); j++)
                       for (k = 0; k < ARRAY_SIZE(probs->coef[0][0]); k++)
                               update_coeff(deltas->coef[i][j][k], 
probs->coef[i][j][k]);
...
static void update_inter_mode_probs(struct v4l2_vp9_frame_context *probs,
                                   const struct 
v4l2_ctrl_vp9_compressed_hdr_probs *deltas)
{
       int i;
...
v4l2_ctrl_vp9_compressed_hdr_probs *deltas)
{
       ...
       for (i = 0; i < ARRAY_SIZE(probs->is_inter); i++)
               probs->is_inter[i] = update_prob(deltas->is_inter[i], 
probs->is_inter[i]);
}
...
v4l2_ctrl_vp9_compressed_hdr_probs *deltas)
{
       ...
       if (reference_mode != V4L2_VP9_REF_MODE_SINGLE)
               for (i = 0; i < ARRAY_SIZE(probs->comp_ref); i++)
                       probs->comp_ref[i] = update_prob(deltas->comp_ref[i], 
probs->comp_ref[i]);
}
...
V4L2_VP9_INTERP_FILTER_SWITCHABLE)
       ...
       update_frame_reference_mode_probs(dec_params->reference_mode, probs, 
deltas);
                           ...
                           struct v4l2_vp9_frame_context *frame_context)
{
       ...
       if (dec_params->flags & V4L2_VP9_FRAME_FLAG_KEY_FRAME ||
           ...
           dec_params->flags & V4L2_VP9_FRAME_FLAG_ERROR_RESILIENT) {
                ...
                * We do nothing here. Instead of storing default probs in some 
intermediate
                * location and then copying from that location to appropriate 
contexts
                * in save_probs() below, we skip that step and save default 
probs directly
...
static u8 merge_prob(u8 pre_prob, u32 ct0, u32 ct1, u16 count_sat, u32 
max_update_factor)
{
...
/*
 * merge_probs() is a recursive function in the spec. We avoid recursion in 
the kernel.
...
static inline void adapt_probs_variant_a_coef(u8 *p, const u32 *c, u32 
update_factor)
{
       merge_probs_variant_a(p, c, 24, update_factor);
}

static inline void adapt_probs_variant_b_coef(u8 *p, const u32 *c, u32 
update_factor)
{
...
(*counts->eob[i][j][k][l][m])[1],
                       ...
                       adapt_probs_variant_a_coef(p, 
*counts->coeff[i][j][k][l][m], uf);
...
adapt_prob(probs->single_ref[i][j],
                                                                    
(*counts->single_ref)[i][j]);

       for (i = 0; i < ARRAY_SIZE(probs->inter_mode); i++)
               adapt_probs_variant_c(probs->inter_mode[i], 
(*counts->mv_mode)[i]);
       ...
       for (i = 0; i < ARRAY_SIZE(probs->partition); i++)
               adapt_probs_variant_e(probs->partition[i], 
(*counts->partition)[i]);
       ...
       if (interpolation_filter == V4L2_VP9_INTERP_FILTER_SWITCHABLE)
               for (i = 0; i < ARRAY_SIZE(probs->interp_filter); i++)
                       adapt_probs_variant_f(probs->interp_filter[i], 
(*counts->filter)[i]);

       if (tx_mode == V4L2_VP9_TX_MODE_SELECT)
               for (i = 0; i < ARRAY_SIZE(probs->tx8); i++) {
                       adapt_probs_variant_b(probs->tx8[i], 
(*counts->tx8p)[i]);
                       adapt_probs_variant_f(probs->tx16[i], 
(*counts->tx16p)[i]);
                       adapt_probs_variant_e(probs->tx32[i], 
(*counts->tx32p)[i]);
               }
       ...
       for (i = 0; i < ARRAY_SIZE(probs->mv.sign); i++) {
               probs->mv.sign[i] = adapt_prob(probs->mv.sign[i], 
(*counts->sign)[i]);

               adapt_probs_variant_g(probs->mv.class[i], 
(*counts->classes)[i]);

               probs->mv.class0_bit[i] = adapt_prob(probs->mv.class0_bit[i], 
(*counts->class0)[i]);
...
MODULE_AUTHOR("Andrzej Pietrasiewicz <andrze...@collabora.com>");
